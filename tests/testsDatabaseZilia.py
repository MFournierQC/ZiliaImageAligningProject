...
class TestZilia(env.DCCLabTestCase):
    ...
    # def testDictValuesAreAList(self):
    #     images = self.db.getGrayscaleEyeImages(monkey='Bresil' , rlp=6, timeline='baseline 3', region='onh', limit=10)
    #     images[0] 

    #     images = self.db.getRGBImages(monkey='Bresil' , rlp=6, timeline='baseline 3', region='onh', limit=10)
    #     images[0]

    # @unittest.skip("Large images")
    ...
    # def testComputeThingsOnManyImageDataMultiprocessing(self):
    #     images = self.db.getGrayscaleEyeImagesWithPaths(limit=10)
    #     self.assertIsNotNone(images)

    #     allImageData = list(images.values())
        
    #     with Pool(5) as p:
    #         results = p.map(self.computeSomething, allImageData)
    #         print(results)
