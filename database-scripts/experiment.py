...
class Curve:
    ...
    def __repr__(self):
        if self.dx is not None and self.dy is not None:
            ...
            return "{0}±{1} {2}".format(self.x, self.dx, self.y)
        elif self.dy is not None :
            return "{0} {1}±{2}".format(self.x, self.y, self.dy)
        else:
            string = ""
...
class DataFile:
    def __init__(self, filepath, columnId=None, delimiter = None, hasHeader=None, skipLines=0):
        self.filepath = filepath
        self.delimiter = delimiter
        self.headers = []
        self.rows = self.readRows(self.filepath, delimiter=self.delimiter, hasHeader=hasHeader)
    ...
    def readRows(self, filepath, delimiter=None, hasHeader=None, dialect=None):
        rows = []
        with open(filepath) as csvfile:
            if delimiter is None and dialect is None:
                dialect = csv.Sniffer().sniff(csvfile.read(1024))

            csvfile.seek(0)
            if hasHeader :
                # or csv.Sniffer().has_header(csvfile.read(1024))
                csvfile.seek(0)
                self.headers = csvfile.readline().rstrip().split(delimiter)

                while True:
                    current = csvfile.tell()
                    if len(csvfile.readline().rstrip().split(delimiter)) == len(self.headers):
                        csvfile.seek(current)
                        break
    ...
    def classifyColumns(self, identification):
        ...
        if identification is not None:
            if len(identification.split()) == len(self.headers):
        ...
        for i in range(len(self.headers)):
            if i == 0:
                self.columns[i].role = f'x0'
            else:   
                self.columns[i].role = f'y{i-1}'

            if len(self.columns[i].label) == 0:
                self.columns[i].label = self.columns[i].role
            dict[self.columns[i].role] = self.columns[i]
